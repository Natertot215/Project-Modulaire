import type { Module, SessionType } from "../types";

export const buildModules = (type: SessionType, total: number): Module[] => {
  if (type === "practice") {
    return [{ label: "Practice", sec: "mixed", count: total, start: 0 }];
  }
  if (total === 98) {
    return [
